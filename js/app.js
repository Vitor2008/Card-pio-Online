...
var CELULAR_EMPRESA = '5565912345678';
...
cardapio.metodos = {
    ...
    // Atualizar o badge de totais dos botões "Menu carrinho"  
    atualizarBadgeTotal: () => {
    ...
    resumoPedido: () => {
        ...
        if (uf == "-1") {
            cardapio.metodos.mensagem('Informe a UF por favor, por favor.');
            $("#ddlUF").focus();
            return;
        }

        if (numero.length <= 0) {
            cardapio.metodos.mensagem('Informe o Número por favor, por favor.');
            $("#txtNumero").focus();
            return;
        }
    ...
    // carrega a etapa resumo dos itens
    carregarResumo: () => {
    ...
    // carregar link do botão WhatsApp
    carregarBotaoWhatsApp: () => {

        var texto = 'Olá! vim através do *site*, e gostaria de saber mais a respeito.';
     ...
     // carregar link do botão WhatsApp footer
